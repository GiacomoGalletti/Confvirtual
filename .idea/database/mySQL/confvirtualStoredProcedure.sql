...
CREATE PROCEDURE ritornaConferenzeFuture(IN in_dataOggi date)
BEGIN
	select *
    from CONFERENZA
...
CREATE PROCEDURE ritornaConferenzePassate(IN in_dataOggi date)
BEGIN
	select *
    from CONFERENZA
    where (CONFERENZA.acronimo,CONFERENZA.annoEdizione) IN (
...
BEGIN  
	insert into conferenza(codice,oraInizio,oraFine,titolo,linkStanza,numeroPresentazioni,giornoData,annoEdizioneConferenza,acronimoConferenza) values (in_codice,
